...
<script>
    $(function(){
        ...
        // Disable options based on selected status
        $('#status').change(function() {
            var selectedStatus = $(this).val();
            $('#status option').prop('disabled', false);
            
            if (selectedStatus == 1) { // If Cancelled is selected
                $('#status option[value="2"]').prop('disabled', true); // Disable Confirmed
                $('#status option[value="3"]').prop('disabled', true); // Disable Delivered
            } else if (selectedStatus == 2) { // If Confirmed is selected
                $('#status option[value="0"]').prop('disabled', true); // Disable Pending
                $('#status option[value="1"]').prop('disabled', true); // Disable Cancelled
                $('#status option[value="3"]').prop('disabled', true); // Disable Delivered
            } else if (selectedStatus == 3) { // If Delivered is selected
                $('#status option').prop('disabled', true); // Disable all options
            }
        });
    })
</script>
